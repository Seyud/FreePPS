...
#[cfg(unix)]
use crate::common::constants::{BATTERY_STATUS_PATH, MODULE_PROP, PD_ADAPTER_VERIFIED_PATH};
...
impl ModuleManager {
    ...
    fn apply_mode(&self, mode: ChargingMode) -> Result<()> {
        ...
        if Path::new(PD_VERIFIED_PATH).exists() {
            ...
            let should_write = public_pps || self.is_idle();
            if should_write {
                ...
                info!("设备已连接，切换到 {:?} 时不强制清零qcom节点", mode);
            }
        }
    ...
    fn is_idle(&self) -> bool {
        FileMonitor::read_file_content(BATTERY_STATUS_PATH).unwrap_or_default() == "Discharging"
    }
