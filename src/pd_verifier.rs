...
impl PdAdapterVerifier {
    ...
    /// 读取当前PD适配器验证状态
    #[allow(dead_code)]
    pub fn read_pd_adapter_verified(&self) -> Result<bool> {
        let content = FileMonitor::read_file_content(PD_ADAPTER_VERIFIED_PATH)?;
        Ok(content == "1")
    }

    /// 维护PD适配器验证状态（检查并重置为1）
    #[allow(dead_code)]
    pub fn maintain_pd_adapter_verified(&self) -> Result<()> {
        let current_value = self.read_pd_adapter_verified()?;

        if !current_value {
            info!("PD适配器验证状态为0，重新设置为1");
            self.set_pd_adapter_verified(true)?;
        } else {
